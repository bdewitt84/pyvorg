...
class MoveVideoCmd(Command):
    ...
    def _update_target_file_path(self):
        self.target_subdir = generate_destination_path(self.video, self.target_root,
                                                       self.format_string) / self.video.get_filename()
    ...
    def validate_exec(self) -> tuple[bool, list[str]]:
        self._update_target_file_path()
        return fileutils.validate_move(self.video.get_path(), self.target_file_path.parent)
